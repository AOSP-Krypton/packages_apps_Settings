...
public class PreventRingingParentPreferenceController extends TogglePreferenceController
    ...
    public boolean isChecked() {
        ...
        if (preventRinging == null) preventRinging = KOSP_VOLUME_HUSH_OFF;
        return !preventRinging.equals(KOSP_VOLUME_HUSH_OFF);
    }
    ...
    public boolean setChecked(boolean isChecked) {
        String preventRingingSetting = Settings.Secure.getString(
                mContext.getContentResolver(), VOLUME_HUSH_GESTURE);
        if (preventRingingSetting == null) preventRingingSetting = KOSP_VOLUME_HUSH_OFF;

        final String newRingingSetting = preventRingingSetting.equals(KOSP_VOLUME_HUSH_OFF)
                ? KOSP_VOLUME_HUSH_VIBRATE : preventRingingSetting;

        return Settings.Secure.putString(mContext.getContentResolver(),
                VOLUME_HUSH_GESTURE, isChecked
                        ? newRingingSetting
                        : KOSP_VOLUME_HUSH_OFF);
    }
    ...
    public void updateState(Preference preference) {
        ...
        if (value == null) value = KOSP_VOLUME_HUSH_OFF;
        String summary = null;
        if (isVolumePowerKeyChordSetToHush()) {
            if (value.equals(KOSP_VOLUME_HUSH_OFF)) {
